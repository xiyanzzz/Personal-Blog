...
5. 用近似随机梯度做更新：$\theta\prime\leftarrow\theta+\beta\cdot \sum_{t=1}^n\gamma^{t-1}\cdot u_t\cdot\nabla_{\boldsymbol{\theta}}\ln\pi(a_t|s_t;\boldsymbol{\theta}). $
...
5. 更新策略网络$\pi(a|s;\theta)$:
   ...
   - 随机梯度**下降**更新策略网络：$\theta\prime\leftarrow\theta-\beta\cdot \sum_{t=1}^n\gamma^{t-1}\cdot \delta_t\cdot\nabla_{\boldsymbol{\theta}}\ln\pi(a_t|s_t;\boldsymbol{\theta}). $
